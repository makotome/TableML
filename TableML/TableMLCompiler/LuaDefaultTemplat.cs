...
function TableFile:LoadFromFile(file)
    ...
    if self.fileFullPath == nil then
        self.fileFullPath = ""Product/""..file
    end
...
function TableFile:parseReader(path)
    ...
    local firstLineSplitString = headLine:split(self.Separator)
    local firstLineDef = { }
    local metaLineArr = metaLine:split(self.Separator)
    ...
    while sLine ~= nil do
        ...
        if sLine ~= nil then
            local splitString1 = sLine:split(self.Separator)
...
function {{file.ClassName}}Settings:ReloadAll()
    ...
    if CS.KEngine.Log.IsUnityEditor ~= true then
        path = CS.KEngine.KResourceModule.BundlesPathRelative..""Setting/""..self.tabFilePath
    else
        path = ""Setting/""..self.tabFilePath
    end
